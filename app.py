...
def run_analysis(nlp_choice: str, df, news_source: str):
    titles = df["title"]
    ...
    if nlp_choice == "Intro":
        intros = {
            ...
            """,
            "Forbes": """
            Forbes is an American business magazine founded in 1917 by B.C. Forbes.
            It covers business, investing, technology, entrepreneurship, leadership,
            and lifestyle topics for a global audience.
            """,
            "Investing.com": """
            Investing.com is a financial markets platform and news website offering
            market quotes, stock and commodity analysis, and breaking news covering
            forex, equities, and the global economy.
            """,
        }
        st.markdown(intros.get(news_source, f"### {news_source}"))
...
if df.empty:
    st.error(
        f"Не вдалося отримати жодної статті з джерела «{s_news}». "
        "Можливо, сайт тимчасово блокує запити або змінив формат RSS. "
        "Спробуйте інше джерело новин."
    )
else:
    run_analysis(s_nlp, df, s_news)
